...
for outer_iter in range(job_data['num_iter']):
    ...
    # Mohak - commenting this out since we are only running MPC
    # if job_data['start_state'] == 'init':
    #     print('sampling from initial state distribution')
    #     buffer_rand_idx = np.random.choice(
    #         len(init_states_buffer), size=job_data['update_paths'], replace=True).tolist()
    ...
    #         num_states_1 = int(
    #             job_data['update_paths']*(1-job_data['buffer_frac'])) + 1
    #         num_states_2 = int(
    #             job_data['update_paths'] * job_data['buffer_frac']) + 1
    #     else:
    #         num_states_1, num_states_2 = job_data['update_paths'] // 2, job_data['update_paths'] // 2
    #     buffer_rand_idx = np.random.choice(
    #         len(init_states_buffer), size=num_states_1, replace=True).tolist()
